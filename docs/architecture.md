...
The browser uses the Supabase publishable key only. The Supabase secret key and all LLM credentials remain in the API. The model may propose only allow-listed tools and receives no arbitrary SQL capability. Mutating voice actions return a preview and require confirmation.
